...
const UnitUsaha = ({ data }) => {
  ...
  let [hasProduct, setHasProduct] = useState(data.unitUsaha.data[0].products.length != 0 );
  let unitUsaha = data.unitUsaha.data;
  const router = useRouter()
  let handleChange = async (data) => {
    let unitUsaha = await axios.get('/api/profil/' + data.id);
    setProfil(unitUsaha?.data)
    setHasProduct(data.products.length != 0)
  }
